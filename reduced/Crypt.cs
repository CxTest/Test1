...
*/
                                        ...
                                        byte[] password, int length)
        {
            ...
            if (fromIndex < 0 || fromIndex >= from.Length)
					throw new ArgumentException(MySqlResources.IndexMustBeValid, "fromIndex");
            if ((fromIndex + length) > from.Length)
					throw new ArgumentException(MySqlResources.FromAndLengthTooBig, "fromIndex");
            if (from == null)
					throw new ArgumentException(MySqlResources.BufferCannotBeNull, "from");
            if (to == null)
					throw new ArgumentException(MySqlResources.BufferCannotBeNull, "to");
            if (toIndex < 0 || toIndex >= to.Length)
					throw new ArgumentException(MySqlResources.IndexMustBeValid, "toIndex");
            if ((toIndex + length) > to.Length)
					throw new ArgumentException(MySqlResources.IndexAndLengthTooBig, "toIndex");
            if (password == null || password.Length < length)
					throw new ArgumentException(MySqlResources.PasswordMustHaveLegalChars, "password");
            if (length < 0)
					throw new ArgumentException(MySqlResources.ParameterCannotBeNegative, "count");
